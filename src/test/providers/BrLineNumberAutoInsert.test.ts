...
suite('Auto Line Number Insertion Test Suite', () => {
	...
	test('Return null when previous line has line number but no content', async () => {
		console.log('Running previous line with no content test...')
		const document = await vscode.workspace.openTextDocument({
			language: 'br',
			content: '00100 print "first"\n00110\n'
		})

		// Line 0: 00100 print "first" (has content)
		// Line 1: 00110 (no content, just line number)
		// Line 2: (blank line where cursor would be after pressing Enter)
		// Should return null because previous line (line 1) has no content
		const nextLineNum = calculateNextLineNumber(parser, document, 2, 10, 5)

		assert.strictEqual(nextLineNum, null, 'Should return null when previous line has no content')

		console.log('Previous line with no content test passed')
		await vscode.commands.executeCommand('workbench.action.closeActiveEditor')
	})

	test('Add line number when previous line has content', async () => {
		console.log('Running previous line with content test...')
		const document = await vscode.workspace.openTextDocument({
			language: 'br',
			content: '00100 print "first"\n00110 print "second"\n'
		})
		...
		// Line 0: 00100 print "first" (has content)
		// Line 1: 00110 print "second" (has content)
		// Line 2: (blank line where cursor would be after pressing Enter)
		// Should add line number because previous line (line 1) has content
		const nextLineNum = calculateNextLineNumber(parser, document, 2, 10, 5)

		assert.strictEqual(nextLineNum, '00120', 'Should add line number when previous line has content')

		console.log('Previous line with content test passed')
		await vscode.commands.executeCommand('workbench.action.closeActiveEditor')
	})

	test('Skip blank line numbers and find last line with content', async () => {
		console.log('Running skip blank line numbers test...')
		const document = await vscode.workspace.openTextDocument({
			language: 'br',
			content: '00100 print "first"\n00110\n00120\n'
		})
		...
		// Line 0: 00100 print "first" (has content)
		// Line 1: 00110 (no content)
		// Line 2: 00120 (no content)
		// Line 3: (blank line where cursor would be after pressing Enter)
		// Should walk back to line 0 and use its line number
		const nextLineNum = calculateNextLineNumber(parser, document, 3, 10, 5)

		assert.strictEqual(nextLineNum, '00110', 'Should skip blank line numbers and find 00100')

		console.log('Skip blank line numbers test passed')
		await vscode.commands.executeCommand('workbench.action.closeActiveEditor')
	})

	test('Add line number after comment line', async () => {
		console.log('Running add after comment test...')
		const document = await vscode.workspace.openTextDocument({
			language: 'br',
			content: '00100 print "first"\n00110 ! this is a comment\n'
		})

		// Line 0: 00100 print "first" (has content)
		// Line 1: 00110 ! this is a comment (has content - comments count as content)
		// Line 2: (blank line where cursor would be after pressing Enter)
		// Should add line number because comments are content
		const nextLineNum = calculateNextLineNumber(parser, document, 2, 10, 5)

		assert.strictEqual(nextLineNum, '00120', 'Should add line number after comment line')

		console.log('Add after comment test passed')
		await vscode.commands.executeCommand('workbench.action.closeActiveEditor')
	})
})
