...
import { Connection, Diagnostic, DiagnosticSeverity, DocumentSymbol, Position, Range, SymbolKind, TextDocumentChangeEvent, TextDocuments} from 'vscode-languageserver/node';
...
export default class BrParser implements Disposable {
  ...
  getBufferTree(uri: Uri, buffer: ArrayBufferLike) {
	...
	updateTree(document: TextDocument, contentChanges: TextDocumentContentChangeEvent[]){
		...
		if (oldTree){
			for (const change of contentChanges) {
				...
				if (newRow === change.range.start.line){
					...
					endPos.column = newLines[newLines.length - 1].length
				}
		...
		if (tree){
          ...
          const range: Range = {
            start: {
              ...
              character: capture.node.endPosition.column
            }
          }
					diagnostics.push({
						code: '',
	...
	getNodeAtPosition(doc: SourceDocument, position: Position): Parser.SyntaxNode | null {
		const node = doc.getTree().rootNode.namedDescendantForPosition(this.getPoint(position))
		return node
	}
		...
		for (const error of errors) {
			for (const capture of error.captures) {
				diagnostics.push({
					...
					range: this.getNodeRange(capture.node),
					severity: DiagnosticSeverity.Error,
					source: 'BR Syntax Scanner',
					// relatedInformation: [
